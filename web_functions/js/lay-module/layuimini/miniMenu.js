...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        compileMenu: function(menu,isSub){
            var menuHtml = '<li {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.childOpenClass}} {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a {{#if( d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="javascript:;">{{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav">{{d.title}}</span>'+ (menu.children ? '<span class="layui-badge layui-bg-gray">'+ $(menu.children).children().length +'</span>': '') +'</a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>' ;
            if(isSub){
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            $('.layuimini-menu-left').append(leftMenuHtml);
